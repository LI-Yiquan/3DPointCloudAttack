...
parser.add_argument(
    '--model', type=str, default='PointNet++Msg', help='model type: PointNet or PointNet++Ssg or PointNet++Msg or '
                                                       'DGCNN or CurveNet')
parser.add_argument(
...
classifier.load_state_dict(
        torch.load(os.path.expanduser(
            '~//yq_pointnet//cls//{}//{}_model_on_{}.pth'.format(opt.dataset, opt.model,opt.dataset))))
...
for epoch in range(opt.nepoch):
    ...
    print('test accu: %.2f' % (
         total_correct / float(total_testset)))
    '''

...
from iso_utils import progress_bar, adjust_lr_steep, log_row
...
def gradient_attack(obj, label, model, args):
    ...
    optimizer = torch.optim.Adam(filter(lambda p: p.requires_grad, model.parameters()), lr=args.step_size,
                                 betas=(0.9, 0.999))
...
def gen_attack_log(args):
    ...
    with open(logname, 'a') as logfile:
        ...
        logwriter.writerow(['model type', 'data set', 'random seed',
                            'number of points in one batch', 'number of points in one object', 'model load path',
                            'steps of gradient-like attack',
                            'step size (lr) fo gradient-like attack', 'number of test objects', 'penalty coefficient',
                            'target or not', 'kappa for CW', 'number of repeat initial attacks',
...
if __name__ == '__main__':
    ...
    parser.add_argument('--target', type=int, default=1,
                        help='whether use a targeted label or not (CW/CE loss), 0 means no target')
    ...
    parser.add_argument('--num_class', type=int, default=105+1, help='number of class')
    ...
    parser.add_argument('--dataset', type=str, default='Bosphorus', help='dataset')
    ...
    if args.model == 'PointNet':
        model = PointNetCls(k=args.num_class, feature_transform=False)
    elif args.model == 'PointNet2_MSG':
        model = PointNet_Msg(args.num_class, normal_channel=False)
    elif args.model == 'PointNet2_SSG':
        model = PointNet_Ssg(args.num_class)
    ...
    for i in range(args.num_tests):
        ...
        obj = obj.type(torch.FloatTensor).to(device)
        ...
        if correct == 0:
            ...
            if (correct == 0) and (true_prob_before > 0.98 and rates[0].item() > 0.98 or penalty < 0.05) and (
                    save_times <= 10):
                save_times += 1

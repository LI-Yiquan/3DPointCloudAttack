...
#        self.attack_method = args.transfer_attack_method if args.query_attack_method is None else args.query_attack_method
        self.attack_method = args.query_attack_method
    ...
    def build_models(self,args):
        ...
        if args.surrogate_model == 'PointNet':
            ...
            surrogate_model = DGCNN(args,output_channels=num_of_class).to(args.device)
        elif args.surrogate_model == 'CurveNet':
            surrogate_model = CurveNet(num_classes=num_of_class)
        else:
        ...
        if args.target_model == 'PointNet':
            ...
            target_model = DGCNN(args,output_channels=num_of_class).to(args.device)
        elif args.target_model == 'CurveNet':
            target_model = CurveNet(num_classes=num_of_class)
        else:
            exit('wrong target model type')

        surrogate_model.load_state_dict(
            torch.load((os.path.expanduser(
                '~//yq_pointnet//cls//Bosphorus//{}_model_on_Bosphorus.pth'.format(args.surrogate_model)))))
        target_model.load_state_dict(
            torch.load((os.path.expanduser(
                '~//yq_pointnet//cls//Bosphorus//{}_model_on_Bosphorus.pth'.format(args.target_model)))))
        wb_classifier = surrogate_model.to(self.args.device)
        classifier = target_model.to(self.args.device)
        # load model weights
    ...
    def shape_invariant_ifgm(self, points, target):
        ...
        for i in range(self.max_steps):
            ...
            if not self.defense_method is None:
                logits,_,_ = self.wb_classifier(self.pre_head(points))
            else:
                logits,_,_ = self.wb_classifier(points)
        ...
        with torch.no_grad():
            ...
            if not self.defense_method is None:
                adv_logits,_,_ = self.classifier(self.pre_head(points.transpose(1, 2).detach()))
            else:
                adv_logits,_,_ = self.classifier(points.transpose(1, 2).detach())
    ...
    def simba_attack(self, points, target):
        ...
        with torch.no_grad():
            ...
            if not self.defense_method is None:
                adv_logits,_,_ = self.classifier(self.pre_head(points.detach()))
            else:
                adv_logits,_,_ = self.classifier(points)
        ...
        while best_loss < 0 and i < len(basis_list):
            ...
            for eps in {self.step_size, -self.step_size}:
                ...
                with torch.no_grad():
                    if not self.defense_method is None:
                        logits,_,_ = self.classifier(self.pre_head(inputs.detach()))
                    else:
                        logits,_,_ = self.classifier(inputs.detach()) # [1, num_class]
                    query_costs += 1
    ...
    def shape_invariant_query_attack(self, points, target):
        ...
        # normal_vec = points[:,:,-3:].data # N, [1, N, 3]
        # print("target: ",target)
        normal_vec = self.get_normal_vector(points)
        normal_vec = normal_vec / torch.sqrt(torch.sum(normal_vec ** 2, dim=-1, keepdim=True)) # N, [1, N, 3]
        points = points[:,:,:3].data # P, [1, N, 3]
        ori_points = points.data
        # initialization
        ...
        # get white-box gradients
        logits,_,_ = self.wb_classifier(inputs)
        loss = self.CWLoss(logits, target, kappa=-999., tar=True, num_classes=self.num_class)
        self.wb_classifier.zero_grad()
        loss.backward()
        ...
        if self.top5_attack:
            ...
            if target in target_top_5:
                ...
                adv_target = -1
        del grad
        shuffled_adv_points = rand_row(adv_points)
        shuffled_adv_points = shuffled_adv_points.transpose(1,2)
        shuffled_adv_points = shuffled_adv_points.cuda().float()
        with torch.no_grad():
            if not self.defense_method is None:

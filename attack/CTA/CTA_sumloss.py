...
def act_max(network,
    ...
    if target_att == 'random':
        ...
        while (tar_cls == ori_cls):
            tar_cls = random.randint(0, 104)
        print('ori:', ori_cls)
        print('Target class (random) is ', tar_cls)
        ...
        print('The second max activation is ,', tar_cls)

    elif target_att == 'least':
        if torch.cuda.is_available() == True:
            tar_cls = torch.topk(layer_activation[layer_name][0], 105).indices[-1].cpu().detach().numpy()
        else:
            tar_cls = torch.topk(layer_activation[layer_name][0], 105).indices[-1].detach().numpy()
        print('Lowest activation is , ', tar_cls)
    ...
    for num_p_per in range(n_points, np.sum(contri > 0)):
        ...
        while (True):
            ...
            unit_to_opt = ori_cls
            gradient_optimizer = torch.tensor(0).cuda().float()
            if using_softmax_neuron == True:
                for j in range(set_size):
                    sftmax = torch.nn.functional.softmax(layer_out[j])
                    ls = torch.log(sftmax)
                    gradient_optimizer = alpha * ls[unit_to_opt]
                gradient_optimizer = gradient_optimizer / set_size

            elif using_softmax_neuron == False:
                if target_att != False:
                    for j in range(set_size):
                        gradient_optimizer = alpha * (layer_out[j][unit_to_opt] - layer_out[j][tar_cls])
                    gradient_optimizer = gradient_optimizer/set_size
                elif target_att == False:
                    # gradient_optimizer = alpha * (layer_out[0][unit_to_opt] - layer_out[0][torch.topk(layer_out[0], 2).indices[-1]])
                    for j in range(set_size):
                        gradient_optimizer += alpha * (layer_out[j][unit_to_opt])
                    gradient_optimizer = gradient_optimizer/set_size
            ...
            if penalize_dis == True:
                ...
                for pa in range(num_p_per):
                    ...
                    total_dis += dis_utils_torch.sgd_hausdorff_dis(
                        input[0, :, contr_index[pa]].unsqueeze(0).unsqueeze(-1),
                        prototype[0, :, contr_index[pa]].unsqueeze(0).unsqueeze(-1))
            ...
            for j in range(set_size):
                for pa in range(num_p_per):
                    input_grad_masked[j, :, contr_index[j][pa]] = input_grad[j, :, contr_index[j][pa]]
            ...
            if verbose:
                ...
                if target_att == False:
                    print("\r", 'step: ', step, 'cur step: ', cur_step, 'ori_cls activation: ', to_display_ori, end="",
                          flush=True)
                elif target_att != False:
                    if torch.cuda.is_available() == True:
            ...
            if target_att == False:
                if cur_class != ori_cls:
                    print('\nAttack success, current prediction class: ', cur_class)
                    return state, best_img, ori_logits, max_other_logits
            elif target_att != False:
                if cur_class == tar_cls:
                    print('\nCurrent prediction: ', cur_class, 'Target class: ', tar_cls,
                          ' Success!')
                    return state, best_img, ori_logits, max_other_logits
            ...
            if (cur_step >= 25 and cur_step % 25 == 0):
                ...
                #          mean_recorder_last_tar, 'tar new: ', mean_recorder_new_tar)
                elif target_att == False:
                    print('ori last: ', mean_recorder_last_ori, 'ori new: ', mean_recorder_new_ori, 'tar last: ')

                if target_att != False:
                    if (mean_recorder_new_ori >= mean_recorder_last_ori) or (
                            mean_recorder_new_tar <= mean_recorder_last_tar) or (cur_step >= 1500):

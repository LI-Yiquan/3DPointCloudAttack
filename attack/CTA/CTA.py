...
import numpy as np
from attack.CTA.utils.integrated_gradients import IntegratedGradients
from attack.CTA.utils import dis_utils_torch
# from attack.CTA.utils.integrated_gradients import IntegratedGradients
...
def act_max(network,
    ...
    logits = network(input)  # [B, num_classes]
    if isinstance(logits, tuple):  # PointNet
        logits = logits[0]
    pred = torch.argmax(logits, dim=1)
    #print('ori classify as: {}'.format(pred))
    ...
    for num_p_per in range(0, np.sum(contri > 0)):
        cur_step = 0
